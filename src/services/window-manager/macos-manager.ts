...
export class MacOSWindowManager extends BaseWindowManager {
    ...
    private readonly MAX_PROCESS_ID = 999999
    private readonly ALLOWED_CHARACTERS = /^[a-zA-Z0-9\s\-_.()[\]{}|:;'"<>?!@#$%^&*+=~/\\]*$/
    private readonly securityLogger = log.child('MacOSWindowManager-Security')
     ...
     */
    private validateProcessId(processId: number): number {
    ...
    getPlatformSpecificApi(): any {
        return {
            ...
            secureOsascript: {
                exec: (script: string, context: string) => this.secureExecAppleScript(script, context)
            },
